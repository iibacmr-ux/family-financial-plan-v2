...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        budget_total_filtre = sum(p['montant_total'] for p in projets_filtered)
        st.markdown(f"**💸 Budget Total:** {format_currency(budget_total_filtre)}")
...
def show_analytics():
    ...
    if projets_filtered:
        ...
        total_row = {
            ...
            'Type': '-',
            'Responsable': '-',
            'Budget Total': df_projets['Budget Total'].sum(),
            'Utilisé': df_projets['Utilisé'].sum(),
            'Progression %': (df_projets['Utilisé'].sum() / df_projets['Budget Total'].sum() * 100) if df_projets['Budget Total'].sum() > 0 else 0,
            'Cash Flow': df_projets['Cash Flow'].sum(),
            'ROI %': df_projets['ROI %'].mean()
        }
        df_projets = pd.concat([df_projets, pd.DataFrame([total_row])], ignore_index=True)
